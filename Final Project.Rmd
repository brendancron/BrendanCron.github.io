...
```{r}
cases_df %>%
  ggplot(aes(x= date, y=log2(cases), group=factor(state))) +
  geom_line()
```

Create a predictor data frame that only looks at data up until April 17th.
```{r}
predictor_df <- cases_df %>%
  filter(date < "2020-04-17")
predictor_df
```


Spread the data frame out and fix the NA entries
```{r}
wide_df <- predictor_df %>%
  select(state, date, cases) %>%
  tidyr::spread(date, cases)
