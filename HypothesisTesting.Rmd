...
corona_state_data <- add_term("sourdough")
...
We plan to predict the number of cases for May 17th looking at all of the data up until a month ago.

Create a data frame looking only at number of cases 
```{r}
...
Hypothesis: There is a significant difference between the number of "bread" hits on google when a state is in lockdown versus when it is not in lockdown.

First create a plot to show the relationship examined for testing.
...
Now we need to determine if this difference in bread hits while in lockdown or not is statistically significant.
...
Create a t-test to test for significance

```{r}
bread_hits_t_test <- t.test(bread_hits ~ in_lockdown, data = corona_state_data)
print(paste("p-value = ", bread_hits_t_test$p.value))
print(bread_hits_t_test$estimate)
```


## Machine Learning 

Make a model that predicts percent max cases based on date
then make a model that predicts percent max cases based on sourdough hits
test statistical significance

Creating model to predict percent max cases based on date
```{r}
date_model <- lm(percent_cases ~ date, data=corona_state_data)
date_model
```
